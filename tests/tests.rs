...
//             self.picture.clone().into(),
//             self.visits.clone().into(),
...
//         Self {
//             is_admin: row.pop().unwrap().into(),
//             balance: row.pop().unwrap().into(),
//             visits: row.pop().unwrap().into(),
//             picture: row.pop().unwrap().into(),
//             name: row.pop().unwrap().into(),
//         }
//     }
//
...
//         vec!["name", "picture", "visits", "balance", "is_admin"]
//     }
//
...
//     fn as_row(&self) -> orm::storage::Row {
//         vec![self.is_tall.into()]
//     }
//
//     fn from_row(mut row: orm::storage::Row) -> Self {
//         Self {
//             is_tall: row.pop().unwrap().into(),
//         }
//     }
//
//     fn table_name() -> &'static str {
...
#[cfg(feature = "test-lifetimes-get")]
#[test]
fn lifetimes_get() {
    let mut conn = Connection::open_in_memory().unwrap();
    let tx = conn.new_transaction().unwrap();
